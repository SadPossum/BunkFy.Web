    ...
    <>
      ...
      <section className="card border border-base-300 bg-base-100 shadow-sm">
        ...
        {members.isLoading ? <LoadingState label="Loading staff" /> : members.error ? <div className="p-6"><ErrorState error={members.error} retry={() => void members.refetch()} /></div> : !items.length ? <div className="p-6"><EmptyState icon={<UsersRound />} title={search || status !== "all" ? "No staff members match" : "No staff profiles yet"} description={search || status !== "all" ? "Try another search or status." : "Create the first staff profile, then assign it to a property."} action={canCreate && !search && status === "all" ? <button type="button" className="btn btn-sm btn-primary" onClick={() => setCreateOpen(true)}>Add staff member</button> : undefined} /></div> : <>
          <div className="hidden overflow-x-auto md:block"><table className="table"><thead><tr className="border-base-300 text-[0.68rem] uppercase tracking-[0.12em] text-base-content/40"><th className="pl-6">Staff member</th><th>Role</th><th>Properties</th><th>Status</th><th className="pr-6" /></tr></thead><tbody>{items.map((member) => <tr key={member.staffMemberId} className={`cursor-pointer border-base-300 transition hover:bg-base-200/70 ${member.staffMemberId === focusedMemberId ? focusedResourceClass : ""}`} onClick={() => selectMember(member.staffMemberId)}><td className="pl-6"><StaffIdentity member={member} /></td><td><p className="text-sm font-medium">{member.jobTitle || "No job title"}</p><p className="mt-1 text-xs text-base-content/45">{member.department || "No department"}</p></td><td><span className="badge badge-ghost font-semibold">{member.assignments.length} current</span></td><td><StatusBadge status={staffStatusLabel(member.status)} /></td><td className="pr-6 text-right"><button type="button" className="btn btn-circle btn-ghost btn-xs" onClick={(event) => { event.stopPropagation(); selectMember(member.staffMemberId); }} aria-label={`Open ${member.displayName}`}><ChevronRight size={17} /></button></td></tr>)}</tbody></table></div>
          <div className="divide-y divide-base-300 md:hidden">{items.map((member) => <button key={member.staffMemberId} type="button" className={`block w-full p-5 text-left transition hover:bg-base-200 ${member.staffMemberId === focusedMemberId ? focusedResourceClass : ""}`} onClick={() => selectMember(member.staffMemberId)}><div className="flex items-start justify-between gap-3"><StaffIdentity member={member} /><StatusBadge status={staffStatusLabel(member.status)} /></div><div className="mt-4 flex items-center justify-between gap-3 text-xs text-base-content/50"><span>{member.jobTitle || member.department || "No role details"}</span><span>{member.assignments.length} properties <ChevronRight className="inline" size={15} /></span></div></button>)}</div>
...
function StaffDetail({ memberId, initialTab, properties, selectedProperty, canReadSensitive, canManage, canManageLifecycle, canAssignCurrentProperty, onClose }: { memberId: string | null; initialTab: "profile" | "assignments" | "account"; properties: Property[]; selectedProperty: Property | null; canReadSensitive: boolean; canManage: boolean; canManageLifecycle: boolean; canAssignCurrentProperty: boolean; onClose: () => void }) {
  ...
  useEffect(() => { setTab(initialTab === "account" && !canReadSensitive ? "profile" : initialTab); setEditing(false); setLifecycleAction(null); }, [initialTab, memberId, canReadSensitive]);
  ...
  const profileMutation = useMutation({ mutationFn: ({ item, payload }: { item: StaffMember; payload: Record<string, unknown> }) => request<StaffMember>(`/api/staff/members/${item.staffMemberId}`, { method: "PUT", body: JSON.stringify({ ...payload, expectedVersion: item.version }) }), onSuccess: async () => { setEditing(false); await refresh(); } });
...
function StaffIdentity({ member }: { member: StaffDirectoryMember }) { return <div className="flex min-w-0 items-center gap-3"><InitialAvatar name={member.displayName} size="sm" /><div className="min-w-0"><p className="truncate font-semibold">{member.displayName}</p><p className="mt-1 truncate text-xs text-base-content/45">{member.jobTitle || member.department || "Staff directory"}</p></div></div>; }
