...
export function PropertiesPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Setup" title="Properties" description="Keep each hostel’s physical layout accurate so availability and reservations stay trustworthy." action={<button className="btn btn-primary" onClick={() => setPropertyForm({})}><Plus size={17} />New property</button>} />

      {!workspace.properties.length ? <EmptyState icon={<Building2 />} title="No properties yet" description="Create your first hostel property to begin adding rooms and beds." action={<button className="btn btn-primary" onClick={() => setPropertyForm({})}><Plus size={17} />Add property</button>} /> : (
        <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
          ...
          {selectedProperty && <section className="space-y-6">
            <div className="card border border-base-300 bg-base-100 shadow-sm"><div className="card-body gap-5 p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-3"><h2 className="font-display text-2xl font-semibold">{selectedProperty.name}</h2><StatusBadge status={selectedProperty.status} /></div><p className="mt-2 flex items-center gap-2 text-sm text-base-content/50"><MapPin size={15} />{selectedProperty.timeZoneId} · Code {selectedProperty.code}</p></div><div className="flex gap-2"><button className="btn btn-sm btn-ghost" onClick={() => setPropertyForm({ property: selectedProperty })}><Edit3 size={16} />Edit</button><button className="btn btn-sm btn-ghost text-error" onClick={() => { if (confirm(`Retire ${selectedProperty.name}?`)) retireMutation.mutate({ kind: "property", entity: selectedProperty }); }}><Trash2 size={16} />Retire</button></div></div></div></div>

            <div className="card border border-base-300 bg-base-100 shadow-sm">
              <div className="flex items-center justify-between border-b border-base-300 px-5 py-5 sm:px-6"><div><h2 className="font-display text-xl font-semibold">Rooms & beds</h2><p className="mt-1 text-sm text-base-content/50">The physical topology used by inventory.</p></div><button className="btn btn-sm btn-primary" onClick={() => setRoomForm({})}><Plus size={16} />Add room</button></div>
              {rooms.isLoading ? <LoadingState label="Loading rooms" /> : rooms.error ? <div className="p-6"><ErrorState error={rooms.error} /></div> : !roomItems.length ? <div className="p-6"><EmptyState icon={<Layers3 />} title="No rooms configured" description="Add the first room, then assign beds or sell it as a whole room." action={<button className="btn btn-sm btn-primary" onClick={() => setRoomForm({})}>Add room</button>} /></div> : (
                <div className="grid min-h-[430px] md:grid-cols-[280px_1fr]">
                  ...
                  {selectedRoom && <div className="p-5 sm:p-6"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h3 className="font-display text-xl font-semibold">{selectedRoom.name}</h3><StatusBadge status={selectedRoom.status} /></div><p className="mt-1 text-sm text-base-content/45">{[selectedRoom.buildingLabel, selectedRoom.floorLabel].filter(Boolean).join(" · ") || "No building or floor label"}</p></div><div className="dropdown dropdown-end"><button tabIndex={0} className="btn btn-circle btn-ghost btn-sm" aria-label={`Actions for ${selectedRoom.name}`}><MoreHorizontal size={18} /></button><ul tabIndex={0} className="menu dropdown-content z-10 w-40 rounded-box border border-base-300 bg-base-100 p-2 shadow-lg"><li><button onClick={() => setRoomForm({ room: selectedRoom })}><Edit3 size={15} />Edit room</button></li><li><button className="text-error" onClick={() => { if (confirm(`Retire ${selectedRoom.name}?`)) retireMutation.mutate({ kind: "room", entity: selectedRoom }); }}><Trash2 size={15} />Retire</button></li></ul></div></div>
                    <div className="my-5 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[0.15em] text-base-content/40">Beds</p><button className="btn btn-sm btn-outline" onClick={() => setBedForm({})}><Plus size={15} />Add bed</button></div>
                    {beds.isLoading ? <LoadingState label="Loading beds" /> : beds.error ? <ErrorState error={beds.error} /> : !beds.data?.beds.length ? <EmptyState icon={<BedDouble />} title="No beds in this room" description="Add beds for bed-level sales, or configure the room for room-level inventory." action={<button className="btn btn-sm btn-primary" onClick={() => setBedForm({})}>Add first bed</button>} /> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{beds.data.beds.map((bed) => <div key={bed.bedId} className="rounded-xl border border-base-300 p-4"><div className="flex items-start justify-between"><div className="grid size-9 place-items-center rounded-lg bg-accent/15 text-accent-content"><BedDouble size={17} /></div><div className="dropdown dropdown-end"><button tabIndex={0} className="btn btn-circle btn-ghost btn-xs" aria-label={`Actions for ${bed.label}`}><MoreHorizontal size={15} /></button><ul tabIndex={0} className="menu dropdown-content z-10 w-36 rounded-box border border-base-300 bg-base-100 p-2 shadow-lg"><li><button onClick={() => setBedForm({ bed })}>Edit</button></li><li><button className="text-error" onClick={() => { if (confirm(`Retire bed ${bed.label}?`)) retireMutation.mutate({ kind: "bed", entity: bed }); }}>Retire</button></li></ul></div></div><p className="mt-3 font-semibold">{bed.label}</p><div className="mt-2"><StatusBadge status={bed.status} /></div></div>)}</div>}
                  </div>}
                </div>
              )}
            </div>
      ...
      <PropertyForm state={propertyForm} mutation={propertyMutation} onClose={() => setPropertyForm(null)} />
      <RoomForm state={roomForm} mutation={roomMutation} onClose={() => setRoomForm(null)} />
      <BedForm state={bedForm} mutation={bedMutation} onClose={() => setBedForm(null)} />
    </>
  );
}
