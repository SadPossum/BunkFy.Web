...
export function IntegrationsPage() {
  ...
  return <>
    ...
    <div role="tablist" aria-label="Integration workspace" className="tabs tabs-box mb-5 grid grid-cols-3 bg-base-100 p-1 shadow-sm">{(["connections", "review", "activity"] as const).map((value) => <button key={value} role="tab" aria-selected={tab === value} className={`tab gap-1.5 text-xs font-semibold sm:text-sm ${tab === value ? "tab-active bg-base-200" : ""}`} onClick={() => setTab(value)}>{value === "connections" ? <Cable size={16} /> : value === "review" ? <ClipboardCheck size={16} /> : <Activity size={16} />}{capitalize(value)}</button>)}</div>
    {tab === "connections" && <section className="card border border-base-300 bg-base-100 shadow-sm"><div className="flex flex-col gap-3 border-b border-base-300 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><h2 className="font-display text-xl font-semibold">Connections</h2><p className="mt-1 text-sm text-base-content/50">Configured adapter endpoints and execution modes.</p></div><select className="select select-bordered select-sm w-full sm:w-44" value={status} aria-label="Connection status" onChange={(event) => setStatus(event.target.value as ConnectionStatusFilter)}><option value="all">All statuses</option><option value="enabled">Enabled</option><option value="disabled">Disabled</option></select></div>{connections.isLoading || adapterTypes.isLoading ? <LoadingState label="Loading connections" /> : connections.error || adapterTypes.error ? <div className="p-6"><ErrorState error={connections.error || adapterTypes.error} retry={() => { void connections.refetch(); void adapterTypes.refetch(); }} /></div> : !connectionItems.length ? <div className="p-6"><EmptyState icon={<Cable />} title={status === "all" ? "No connections yet" : `No ${status} connections`} description={status === "all" ? "Create a connection to bring an external reservation source into BunkFy." : "Choose another status filter."} action={canManage && status === "all" ? <button type="button" className="btn btn-sm btn-primary" onClick={() => setCreateOpen(true)}>Create connection</button> : undefined} /></div> : <><div className="divide-y divide-base-300">{connectionItems.map((connection) => <ConnectionRow key={connection.connectionId} connection={connection} onOpen={() => selectConnection(connection.connectionId)} />)}</div><div className="flex items-center justify-between border-t border-base-300 px-4 py-3 sm:px-6"><p className="text-xs text-base-content/45">{connections.data?.totalCount ?? connectionItems.length} total · page {page}</p><div className="join"><button className="btn btn-sm join-item" disabled={page === 1} onClick={() => setPage((current) => current - 1)} aria-label="Previous connections"><ChevronLeft size={16} /></button><button className="btn btn-sm join-item" disabled={page * PAGE_SIZE >= (connections.data?.totalCount ?? 0)} onClick={() => setPage((current) => current + 1)} aria-label="Next connections"><ChevronRight size={16} /></button></div></div></>}</section>}
    {tab === "review" && <ProposalQueue propertyId={selectedPropertyId} canDecide={canDecideProposals} />}
...
function CreateConnectionModal({ open, propertyId, adapterTypes, onClose, onCreated }: { open: boolean; propertyId: string; adapterTypes: AdapterTypeCapability[]; onClose: () => void; onCreated: (connection: AdapterConnection) => Promise<void> }) {
  ...
  return <Modal open={open} title="New integration connection" description="Choose a registered adapter and how it should exchange observations with BunkFy." onClose={onClose}>{adapterTypes.length ? <form className="space-y-5" onSubmit={submit}><label className="form-control block"><span className="label-text mb-2 block text-sm font-semibold">Adapter type</span><select className="select select-bordered w-full" value={adapterType} onChange={(event) => setAdapterType(event.target.value)}>{adapterTypes.map((adapter) => <option key={adapter.adapterType} value={adapter.adapterType}>{adapter.adapterType}</option>)}</select></label><div className="grid gap-4 sm:grid-cols-2"><label className="form-control block"><span className="label-text mb-2 block text-sm font-semibold">Execution mode</span><select className="select select-bordered w-full" value={executionMode} onChange={(event) => setExecutionMode(event.target.value as typeof executionMode)}>{modes.map((mode) => <option key={mode} value={mode}>{capitalize(adapterExecutionModeLabel(mode))}</option>)}</select></label><label className="form-control block"><span className="label-text mb-2 block text-sm font-semibold">Conflict policy</span><select className="select select-bordered w-full" value={conflictPolicy} onChange={(event) => setConflictPolicy(event.target.value as typeof conflictPolicy)}><option value="suggestionsOnly">Suggestions only</option><option value="autoApplyWhenAdapterBaselineUnchanged">Auto-apply safe updates</option></select></label></div><div className="rounded-xl bg-base-200 p-4"><div className="flex items-start gap-3"><Zap size={17} className="mt-0.5 text-primary" /><div><p className="text-sm font-semibold">{conflictPolicy === "suggestionsOnly" ? "Staff review every conflicting update" : "Apply only against an unchanged adapter baseline"}</p><p className="mt-1 text-xs leading-5 text-base-content/50">You can change this policy later without recreating the connection.</p></div></div></div><TextField label="Configuration reference" name="configurationReference" placeholder="config://booking-provider/property-a" /><TextField label="Secret reference (optional)" name="secretReference" placeholder="secret://booking-provider/property-a" required={false} />{mutation.error && <ErrorState error={mutation.error} />}<FormActions submitting={mutation.isPending} submitLabel="Create connection" onCancel={onClose} disabled={!adapterType || !modes.length} /></form> : <div><EmptyState icon={<Cable />} title="No adapter types registered" description="Install or register an adapter capability before creating a connection." /><div className="mt-4 flex justify-end"><button className="btn btn-ghost" onClick={onClose}>Close</button></div></div>}</Modal>;
}

function TextField({ label, name, placeholder, required = true }: { label: string; name: string; placeholder?: string; required?: boolean }) { return <label className="form-control block"><span className="label-text mb-2 block text-sm font-semibold">{label}</span><input className="input input-bordered w-full" name={name} placeholder={placeholder} required={required} /></label>; }
