...
function ConnectionSettingsForm({ connection, capability, submitting, error, onCancel, onSave }: { connection: AdapterConnection; capability?: AdapterTypeCapability; submitting: boolean; error: unknown; onCancel: () => void; onSave: (payload: ConnectionSettingsUpdate) => void }) {
  ...
  return <form className="space-y-4" onSubmit={submit}><div className="grid gap-4 sm:grid-cols-2"><SelectField label="Execution mode" value={mode} onChange={(value) => setMode(value as typeof mode)} options={supportedModes.map((value) => ({ value, label: capitalize(adapterExecutionModeLabel(value)) }))} /><SelectField label="Conflict policy" value={policy} onChange={(value) => setPolicy(value as typeof policy)} options={[{ value: "suggestionsOnly", label: "Suggestions only" }, { value: "autoApplyWhenAdapterBaselineUnchanged", label: "Auto-apply safe updates" }]} /></div><TextInput label="Configuration reference" value={configurationReference} onChange={setConfigurationReference} /><div><TextInput label="New secret reference (optional)" value={secretReference} onChange={setSecretReference} disabled={clearSecret} /><label className="mt-2 flex cursor-pointer items-center gap-2 text-sm"><input className="checkbox checkbox-sm" type="checkbox" checked={clearSecret} onChange={(event) => setClearSecret(event.target.checked)} />Clear the existing secret reference</label></div>{Boolean(error) && <ErrorState error={error} />}<InlineFormActions><button type="button" className="btn btn-ghost btn-sm" onClick={onCancel} disabled={submitting}>Cancel</button><button type="submit" className="btn btn-primary btn-sm" disabled={submitting || !configurationReference.trim()}>{submitting ? <span className="loading loading-spinner loading-xs" /> : <Save size={15} />}Save settings</button></InlineFormActions></form>;
}

function PollingSchedule({ connection, capability, canManage, submitting, error, onConfigure, onCancel, onClear }: { connection: AdapterConnection; capability?: AdapterTypeCapability; canManage: boolean; submitting: boolean; error: unknown; onConfigure: (intervalSeconds: number, maxAttempts: number) => void; onCancel: () => void; onClear: () => void }) {
